...
A dependency-free League of Legends esports dashboard for LCK schedules, live matches, results, and standings.
...
- The dashboard displays all LCK matches in three tabs: Previous (the Monday of the previous calendar week through yesterday), Today, and Next (tomorrow through the Sunday of the following calendar week). The browser requests the combined range in Hong Kong time, which stays within the API's 31-day request limit.
...
Automatic progression covers the discovered LCK stage pages for the current season. When the calendar year changes, the server checks the new `LCK/{year}_Season` page and updates the match history and standings as soon as Leaguepedia publishes it. The API still accepts an optional team filter for reuse by future views; football support is not implemented yet.