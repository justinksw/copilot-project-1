...
- Official schedule entries are decoded from direct, escaped, and nested serialized EventMatch payloads; malformed entries are skipped, while valid team-pair/date/time matches retain their official game IDs. Near-identical start times can resolve a unique match, but ambiguous rematches remain unlinked. Unresolved opponents and impossible series scores are kept upcoming instead of being treated as completed results.
- The dashboard initially loads T1 matches for yesterday, today, and tomorrow. Previous and Next tabs, plus their week navigation buttons, request additional date batches only when selected; every card shows its Hong Kong date and matches are sorted chronologically.
...
- Detail loading records a clear per-game feed state, tries a game's recorded start time before bounded completed-series offsets, and fetches champion/item and live-stat feeds concurrently; an individual feed failure does not hide other games. The browser supplies its already-linked game IDs as a validated fallback when the schedule index has since rotated, and Retry bypasses the short detail cache.
- Schedule batches and normalized match history are cached in versioned `localStorage`, allowing loaded results to remain available during API outages. Match records use official event IDs when available, otherwise a canonical date/team-pair/time identity; changing the batch format discards older cached batches. The server separately caches the fast T1 schedule path, broader stage metadata, and standings data for 10 minutes.
